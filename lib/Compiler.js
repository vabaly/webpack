...
class Compiler {
	...
	/**
	 * 监听模式下开启监听，Watching 里面又调用了 Compiler 的 readRecords 方法
	...
	readRecords(callback) {
		// 1. 首次调用这个函数时，this.recordsInputPath 并不存在
		if (!this.recordsInputPath) {
			this.records = {};
			// 2. 所以会执行 callback
			return callback();
		}
		// 【Todo】不知道在哪个阶段初始化的这个 inputFileSystem 的
		// 但是知道是在 lib/node/NodeEnvironmentPlugin.js 中初始化的
		// this.inputFileSystem 是 CachedInputFileSystem 构造函数创建的，此函数定义在 enhanced-resolve/lib/CachedInputFileSystem 中
		// 【Todo】this.inputFileSystem 是一个类似于 fs 的 API，但是文件是存储在内存中的，但源码还没去分析
